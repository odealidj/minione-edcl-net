...
{
    [HttpGet]
    [ProducesResponseType(typeof(ApiResponse<IReadOnlyList<EDCL.Module.Auth.Application.DTOs.TransporterDto>>), StatusCodes.Status200OK)]
    public async Task<IActionResult> GetList([FromQuery] string? search = null, [FromQuery] int page = 1, [FromQuery] int pageSize = 10, CancellationToken cancellationToken = default)
    {
        ...
        return result.IsSuccess 
            ? Ok(ApiResponse<IReadOnlyList<EDCL.Module.Auth.Application.DTOs.TransporterDto>>.Paginated(result.Value.Items, PaginationMeta.From(result.Value.PageNumber, result.Value.PageSize, result.Value.TotalCount), traceId))
    ...
    public async Task<IActionResult> GetById(long id, CancellationToken cancellationToken)
    {
        ...
        var traceId = HttpContext.TraceIdentifier;
        return result.IsSuccess 
            ? Ok(ApiResponse<EDCL.Module.Auth.Application.DTOs.TransporterDto>.Success(result.Value, traceId))
