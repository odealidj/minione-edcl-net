...
public class AdminCargoController(IMediator mediator) : ControllerBase
{
    ...
    public async Task<IActionResult> GetManifests([FromQuery] string? search = null, [FromQuery] int page = 1, [FromQuery] int pageSize = 10, CancellationToken cancellationToken = default)
    {
        var result = await mediator.Send(new Application.Queries.AdminGetManifests.AdminGetManifestsQuery(search, page, pageSize), cancellationToken);
